...
function plugin (plugins, cb) {
		...
		function plantItSave(src,imagePath) {
			if (RenderingStage === false) {
				DeferredPlanters.push(function(){
					plantItSave(src,imagePath);
				});
			} else {
				RemainingPlanters++;
				var gen = plantuml.generate(src,PlantOptions);
				gen.out.pipe(fs.createWriteStream(path.join(TargetDirectory,imagePath)));
		...
		app.converter.on("resolveBegin", function (context) {
			...
			TargetDirectory = path.join(process.cwd(),app.options.getValue('out'));
		...
		function processComment(text) {
			...
			if(text) {
				while ((match = umlExpression.exec(text)) != null) {
					...
					if (match[2]) {
						var ImgPath = path.join(outputDirectory,'c' + CommentId + Extension);
						segments.push("![");
						if (match[1]) {
							// alternate text
							segments.push(match[1]);
						}
						segments.push("](../" + ImgPath + ")");
						//already start generating the uml image
						plantItSave('@startuml\n' + match[2] + '\n@enduml',ImgPath);
					}
			...
			if (Complete && IndexPageMatcher.exec(page.url)) {
				...
				if (Contents) {
					...
					if (Match) {
						...
						var ImgPath = path.join(outputDirectory,'CompleteClassDiagram' + Extension);
						page.contents = Contents.substring(0,InsertPosition) + '<div class="tsd-panel"><h3>Class Diagram</h3>' + implantImg(ImgPath) + '</div>' + Contents.substring(InsertPosition);
						var CompletePUml = getCompletePUml();
						plantItSave(CompletePUml,ImgPath);
					}
				}
				Complete = false;
			}
					...
					if (Match) {
						...
						if (Hierarchy) {
							...
							if (NeighbourhoodPUml) {
								var ImgPath = path.join(outputDirectory,ModelName + Extension);
								UmlHierarchy = implantImg('../' + ImgPath);
								plantItSave(NeighbourhoodPUml,ImgPath);
							}
						}
