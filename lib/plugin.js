...
function plugin (plugins, pluginEndCallback) {
	...
	var PUmlOTable = [];
	// Counts, how many planters remain, before pluginEndCallback can be called.
	...
	//Counts how many generators are still running
	var RemainingGenerators = 0;
	/**
	 ...
	 *   from the DeferredGenerators array. Usually this will only happen once.
	 *   At the point in time this method has been called for the first time,
	 *   beginRender is passed, AbsoluteOutputDirectory is set, so generatePlantUml
	 *   will not push in any new deferred generators.
	 */
	function checkModuleDone() {
		if (DeferredGenerators.length > 0) {
			for (var i = 0,l = DeferredGenerators.length;i<l;++i) {
				//execute the deferred generator. This will increment RemainingGenerators.
				DeferredGenerators[i]();
			}
			DeferredGenerators.length = 0;
	 ...
	 */
	function getLinkToCoI(ToPUmlO,RelativePath) {
		var Parts = [];
		var TargetDir = '';
		if (ToPUmlO.Kind === 128 /* Class */)     TargetDir = 'classes';
		if (ToPUmlO.Kind === 256 /* Interface */) TargetDir = 'interfaces';
		return path.join(RelativePath,TargetDir,ToPUmlO.Name + '.html');
	}
	...
	function generatePlantUml(Src,ImageName,RelativePath) {
		...
		if (RemoteURL !== undefined) {
			...
			if (AbsoluteOutputDirectory === undefined) {
				//defer
	...
	function visitPUmlO(ThisPUmlO,UpDepth,DownDepth,Visited,Detail) {
		...
		if (ThisPUmlO) {
			...
			if (DownDepth !== 0) {
				...
				PUmls.push(handleExtImplArray(ThisName,ThisPUmlO.ImplementedBy,' ..|> ',0,DownDepth,Visited,ChildrenDetail));
			}
			return PUmls.join('\n');
		}
	}
		...
		if (Complete && IndexPageMatcher.exec(page.url)) {
			...
			if (Contents) {
				...
				if (Match) {
					...
					page.contents = Contents.substring(0,InsertPosition) + '<div class="tsd-panel"><h3>Class Diagram</h3>' + generateUmlContainerHtml(Url) + '</div>' + Contents.substring(InsertPosition);
				}
			}
			...
			if (Contents) {
				...
				if (Match) {
					...
					if (SeperateClassDetailView) {
						...
						DetailedClassBox = '<section class="tsd-panel"><h3>Class Details</h3>' + generateUmlContainerHtml(Url) + '</section>';
					}
					...
					var GraphicHierarchyView = '<section class="tsd-panel tsd-hierarchy"><h3>Hierarchy-Diagram</h3>'
						+ UmlHierarchy
						+ '</section>';
					page.contents = Contents.substring(0,MatchIndex)
						+ (ClassicHierarchyBefore?(ClassicHierarchyView + GraphicHierarchyView):(GraphicHierarchyView + ClassicHierarchyView))
						+ DetailedClassBox
